...
embed_bp = Blueprint('embed', __name__, url_prefix='/api/webhook')

@embed_bp.route('/send_embed', methods = ['GET', 'POST'])
def send_webhook_embed():
    ...
    if not embed_data or not all(key in embed_data for key in ('title', 'description', 'color')):
        return jsonify({"error": "Invalid embed data. Required keys: title, description, color."}), 400
